...
void LaneFollower::setUp() {
	std::cout << "Autonomous driving on. Manual driving off." << std::endl;
...
void LaneFollower::tearDown() {
	...
	if ((m_image != NULL) && m_isSimulation) {
...
bool LaneFollower::readSharedImage(Container &c) {
	...
	if (c.getDataType() == odcore::data::image::SharedImage::ID()) {
		...
		if (m_sharedImageMemory->isValid()) {
			...
			if (m_image == NULL) {
				m_image = cvCreateImage(cvSize(si.getWidth(), si.getHeight()),
						IPL_DEPTH_8U, numberOfChannels);
			}
...
void LaneFollower::processImage() {
	...
	for (int32_t y = m_image->height - 8; y > m_image->height * .6; y -= 10) {
		...
		if (m_debug) {
			if (left.x > 0) {
				...
				cvLine(m_image, cvPoint(m_image->width / 2, y), left, green, 1,
						8);

				stringstream sstr;
				sstr << (m_image->width / 2 - left.x);
				cvPutText(m_image, sstr.str().c_str(),
						cvPoint(m_image->width / 2 - 100, y - 2), &m_font,
						green);
			}
			if (right.x > 0) {
				CvScalar red = CV_RGB(255, 0, 0);
				cvLine(m_image, cvPoint(m_image->width / 2, y), right, red, 1,
						8);

				stringstream sstr;
				sstr << (right.x - m_image->width / 2);
				cvPutText(m_image, sstr.str().c_str(),
						cvPoint(m_image->width / 2 + 100, y - 2), &m_font, red);
			}
		}
	...
	cerr << "Processing time: "
			<< (afterImageProcessing.toMicroseconds()
					- beforeImageProcessing.toMicroseconds()) / 1000.0 << "ms."
			<< endl;
	...
	double timeStep = (currentTime.toMicroseconds()
			- m_previousTime.toMicroseconds()) / (1000.0 * 1000.0);
	...
	if (c.getDataType() == Voice::ID()) {
		...
		if (voice.getType() == "denm") {
			if (c.getSenderStamp() == getIdentifier()) {
				std::cout << "Witness event: ROAD EVENT (CRASH)" << std::endl;
			} else {
				std::cout
						<< "Receive denm event message: ROAD EVENT (CRASH). Adapt vehicle route."
						<< std::endl;
...
odcore::data::dmcp::ModuleExitCodeMessage::ModuleExitCode LaneFollower::body() {
	if (m_isSimulation) {
		...
		cvInitFont(&m_font, CV_FONT_HERSHEY_DUPLEX, hscale, vscale, shear,
				thickness, lineType);
...
//      std::cout << "My vehicle " << c.getSenderStamp() << endl;
				...
				if (c.getDataType() == odcore::data::image::SharedImage::ID()) {
					...
					// Overtaking part.
					{
						...
						if (containerVehicleData.getDataType()
								== automotive::VehicleData::ID()) {
							VehicleData vd = containerVehicleData.getData<
									VehicleData>();
							m_KeyValueAdhocDataStore.erase(
									automotive::VehicleData::ID());
							...
							if (containerSensorBoardData.getDataType()
								...
								SensorBoardData sbd =
										containerSensorBoardData.getData<
												SensorBoardData>();
								m_KeyValueAdhocDataStore.erase(
										automotive::miniature::SensorBoardData::ID());
								...
								if (stageMeasuring == FIND_OBJECT_INIT) {
									...
									if ((sbd.getValueForKey_MapOfDistances(
											INFRARED_FRONT_RIGHT) > 0)
											&& (sbd.getValueForKey_MapOfDistances(
													INFRARED_REAR_RIGHT) > 0)) {
										// Turn to right.
			...
			cerr << "Send control: acc - "
					...
					<< ", speed - "
					<< std::to_string(m_vehicleControl.getSpeed()) << ", sw - "
					<< std::to_string(m_vehicleControl.getSteeringWheelAngle())
					<< endl;
